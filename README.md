...
- **詰み**: 盤面が満杯で、上下左右に隣接するどの2枚の和も10にならなくなったら終了。終了画面には今回のスコアに加えて、通算の戦数・最高コンボ・最高スコアが表示される。

盤面の大きさは `4×4` `5×5` `6×6` のボタンで切り替えられる(切り替えると新しいゲームが始まる)。`ヒント` は、いま最も多くの組が消える向きを短く示す(盤面の端が一瞬光る)。`もどす` は手を1つずつさかのぼり、`新しいゲーム` でいつでも初めからになる。右上のボタンでテーマを切り替え、共有ボタンを押すと今の配置を再現するリンクがクリップボードにコピーされる。
...
- `src/lib/engine.ts` — スライド、消滅判定、スポーン、手詰まり判定、向きの提案、再開可能な乱数、状態検証の純粋関数
- `src/lib/stats.ts` — 端末に残す通算成績(戦数・最高コンボ・最高スコア)
- `src/lib/index.ts` — engineの公開API
- `src/lib/engine.test.ts` / `src/lib/stats.test.ts` — ロジックのテスト
